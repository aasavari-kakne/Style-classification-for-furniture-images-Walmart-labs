...
def run_classificator(args, res_classificator_model, res_encoder_model, models_store_path):
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in data_loader_train:
            ...
            # check if we have a directory to store extracted features
            if os.path.exists("features")==False:
                # create a new directory to store features
                os.mkdir("features")
                # directory features will have two folders - train and test
                os.mkdir("features/train")
                os.mkdir("features/test")
                # each of the train and test folders will have 16 classes
                for cls_tup in dataset_train.classes:
                    # grab name of every class 
                    cls_name = cls_tup['class_name']
                    # make a directory for that class in train and test folders
                    os.mkdir("features/train/" + cls_name) 
                    os.mkdir("features/test/" + cls_name)

            # grab features only for the first epoch for train class
            if epoch == 0:
                for i in range(img_batch.shape[0]):
                    # grab name of the class and construct path to the class 
                    class_path = "features/train/" + batch.image_list[i]['cls']['class_name']
                    # grab name of the image and store the feature inside the class folder
                    image_path = class_path + "/" + batch.image_list[i]['image_path']
                    # remove excess dimensions
                    np.savetxt(image_path, np.squeeze(patches_encoded[i]))
